...
# observation_dict = {observation: idx for idx, observation in enumerate(observation_list)}
#
# agent_storage = PermanentAgentStorage(env, DynaAgent, experiences=experiences, k=200)
#
...
#plt.axhline(y=np.mean(scores_pagerank), color='r', linestyle='-.', label="DP")
#plt.axhline(y=np.mean(scores_GA_pagerank), color='b', linestyle='-.', label="PageRank GA")

plt.plot(moving_average(scores_pagerank_online_centralized),color='r', linestyle='-', label="DP Online (Centralized)")
plt.plot(moving_average(scores_GA_pagerank_online_centralized),color='b', linestyle='-', label="PageRank GA Online (Centralized)")
